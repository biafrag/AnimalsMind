Shader "Hidden/SC Post Effects/Ripples"
{
	...
	float4 Frag(VaryingsDefault i) : SV_Target
	{
		...
		float x = (center.x - i.texcoord.x) * _Size.x;
		float y = (center.y - i.texcoord.y) * _Size.y;

		float r = -(x * x + y * y);
		float ripples = 1 + 0.5 * sin((r + (_Time.x * _Speed)) / _Distance);

		return SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv + (ripples * _Strength));
	}
	...
	float4 FragPerlin(VaryingsDefault i) : SV_Target
	{
		...
		half ripples = 0.5 + 0.5 * snoise(noiseCoords);
